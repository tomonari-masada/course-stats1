...
\begin{itemize}
\item 特定の観測データ$\bm{x}_{1:T}$に対して、一番あり得そうmost likelyな隠れ状態の列$\bm{z}_{1:T}$を見つけたい
\begin{itemize}
...
\begin{itemize}
\item パラメータ$A, B$の値は与えられていると仮定する
\end{itemize}
\item $\argmax_{\bm{z}_{1:T}} p(\bm{z}_{1:T}|\bm{x}_{1:T}; A,B) = 
\argmax_{\bm{z}_{1:T}} p(\bm{z}_{1:T}, \bm{x}_{1:T}; A,B)$が成り立つことに注意
\item 以下、Viterbiアルゴリズムで$\argmax_{\bm{z}_{1:T}} p(\bm{z}_{1:T}, \bm{x}_{1:T}; A,B)$を求める
\end{itemize}
\end{frame}
...
\begin{itemize}
\item 実は、forward procedureとほとんど同じ計算をする
...
\item termination
\begin{itemize}
\item $v_{\bot} = \max_k v_T(k)$
\item $c_{\bot} = \argmax_k v_T(k)$
\end{itemize}
\item $c_{\bot}$からスタートしてさかのぼれば、隠れ状態の列が得られる
\end{itemize}
\end{frame}
...
\begin{itemize}
\item 観測データの尤度を計算するにしても、与えられた観測データに対して最良の隠れ状態列を得るにしても、
いずれにしても、まずパラメータ$A, B$の値を求めておかないといけない
...
\begin{itemize}
\item E stepでは、モデルパラメータを固定したうえで、観測データを所与とする潜在変数の条件付き確率分布を求めるのだった（cf. 混合分布の講義）
\item ということは、いま考えているHMMの場合、$A$と$B$を固定したうえで$p(\bm{z}_{1:T} | \bm{x}_{1:T})$を計算して、
それを$q(\bm{z}_{1:T})$の解とするのが、E stepとなる
\item 以下、 M stepの説明をするが、そこでは$q(\bm{z}_{1:T})$を$p(\bm{z}_{1:T} | \bm{x}_{1:T})=\frac{p(\bm{z}_{1:T} , \bm{x}_{1:T})}{p(\bm{x}_{1:T})}$で置き換えて計算を進める
\end{itemize}
\end{frame}
...
\notag \\ & \mbox{ \ \ }
+ \sum_{k=1}^K \lambda_k \bigg( 1 - \sum_{l=1}^K A_{s_k, s_l} \bigg)
+ \sum_{k=1}^K \mu_k \bigg( 1 - \sum_{w=1}^W B_{s_k, v_w} \bigg)
\notag \\ &
= 
\sum_{\bm{z}_{1:T}} q(\bm{z}_{1:T}) \sum_{k=1}^K \sum_{l=1}^K \sum_{t=1}^T 
\mathbbm{1}( z_{t-1} = s_k \wedge z_t = s_l ) \ln A_{s_k, s_l}
\notag \\ & \mbox{ \ \ }
...
\FontMath
\begin{align}
\frac{\partial \mathcal{L}(A, B, \lambda, \mu) }{\partial A_{s_k, s_l}}
= \sum_{\bm{z}_{1:T}} q(\bm{z}_{1:T}) \sum_{t=1}^T 
\mathbbm{1}( z_{t-1} = s_k \wedge z_t = s_l ) A_{s_k, s_l}
- \lambda_k
\end{align}
\end{frame}
