...
が成り立つとき、$A$と$B$は$C$が所与のもとで条件付き独立である、という
\item 条件付き確率の定義より$p(A,B|C)=p(A|B,C)p(B|C)$はいつでも成り立つので
\begin{align}
...
& = \sum_{\bm{z}_{1:t-2}} p(z_t|z_{t-1}) \prod_{u=1}^{t-1} p(z_u|z_{u-1})p(x_u|z_u)
\notag \\ & = p(z_t|z_{t-1}) \sum_{\bm{z}_{1:t-2}} \prod_{u=1}^{t-1} p(z_u|z_{u-1})p(x_u|z_u)
\\
p(x_t, z_t,z_{t-1},\bm{x}_{1:t-1}) 
