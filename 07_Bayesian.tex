...
\begin{itemize}
\item 観測データ$\mathcal{D}=\{x_1,\ldots,x_N\}$が独立に同じカテゴリカル分布$\mbox{Cat}(\bm{\phi})$にしたがうと仮定
\begin{itemize}
...
\item 正規ガンマ分布は共役事前分布
\item よって事後分布も正規ガンマ分布となる
\item 事後分布$p(\bm{\phi}|\mathcal{D}; \bm{\beta})$は以下のように書ける
\begin{align}
p(\bm{\phi}|\mathcal{D}; \bm{\beta}) = 
...
\item 上記の設定のもとでの予測分布$p(\bm{x}_0 | \mathcal{D};\bm{\beta})$は
$p(\bm{x}_0|\mathcal{D};\bm{\beta}) = \int p(\bm{x}_0|\bm{\phi})p(\bm{\phi}|\mathcal{D};\bm{\beta}) d\bm{\phi}$によって求められる
\item この予測分布がどういう分布になるかを、以下で示す
\end{itemize}
\end{frame}
...
\FontMath
\begin{align}
& p(\bm{x}_0|\mathcal{D};\bm{\beta}) = \int p(\bm{x}_0|\bm{\phi})p(\bm{\phi}|\mathcal{D};\bm{\beta}) d\bm{\phi}
\notag \\ & =
\int \bigg(
...
\item パラメータ
\begin{itemize}
\item $\alpha_1, \ldots, \alpha_K)$ s.t. $\alpha_k > 0$ for $1 \leq k \leq K$
\end{itemize}
\item 確率密度関数
\begin{align}
...
と見なせる
\end{itemize}
\item 下記の予測分布$p(\bm{x}_0|\mathcal{D})$を使うと、
事後分布$p(\bm{\eta} | \mathcal{D})$によるパラメータへの重み付けを反映するかたちで、
未知データ$\bm{x}_0$の確率を計算できる
\begin{align}
p(\bm{x}_0|\mathcal{D}) = \int p(\bm{x}_0|\bm{\eta}) p(\bm{\eta} | \mathcal{D}) d\bm{\eta}
\end{align}
\end{itemize}
\end{frame}
...
\begin{itemize}
\item 予測分布を求めることは、観測データがしたがう新しい分布を作り出すこと、とも言える
\item 正規分布と正規ガンマ分布から、t location-scale 分布
\item 多項分布とディリクレ分布から、ディリクレ多項分布 
\begin{itemize}
\item ただし、今回の２つの例のように、予測分布の確率密度関数（or 確率質量関数）を式の計算で求めることができてしまうケースは、少ない。
\end{itemize}
\end{itemize}
\end{frame}
